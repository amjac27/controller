...
class DeepinRansomwareSimulator:
    def __init__(self, custom_lock_icon=None):
        ...
        # 使用指定的图片路径
        self.lock_icon_path = custom_lock_icon if custom_lock_icon else "/home/delo/lock.jpg"
...
Comment=Deepin 加密文件 - 教育模拟程序
Icon={self.lock_icon_path}
Exec=echo "这是一个加密文件，请运行解密程序进行恢复。" && zenity --info --text="文件已被加密，请查看 README_DECRYPT_DEEPIN.txt 获取恢复说明。"
Terminal=false
Categories=Utility;
...
<mime-info xmlns="http://www.freedesktop.org/standards/shared-mime-info">
  <mime-type type="application/x-deepin-encrypted">
    ...
    def create_ransom_note(self):
        """创建勒索说明文件"""
        ransom_note = f"""⚠️ 教育模拟程序 - 仅供学习使用 ⚠️
...
def show_menu():
    """显示主菜单"""
    print("\n" + "=" * 60)
    print("Deepin 25 教育模拟程序 - 隐藏文件版")
    print("=" * 60)
    print("1. 模拟加密过程（创建桌面入口）")
    print("2. 模拟解密过程")
    print("3. 查看加密文件状态")
    print("4. 清理配置文件")
    print("5. 退出程序")
...
def main():
    ...
    # 显示免责声明
    print("=" * 70)
    print("⚠️  教育模拟程序 - 仅供学习使用 ⚠️")
    print("=" * 70)
    print("重要提醒:")
    print("1. 请确保您已备份重要数据")
    print("2. 仅在测试环境中使用")
    print("3. 请勿用于非法目的")
    print("4. 加密文件将被隐藏，只显示桌面入口文件")
    print("=" * 70)
    ...
    while True:
        ...
        if choice == "1":
            print(f"\n当前锁图标: {simulator.lock_icon_path}")
            confirm = input("确定要模拟加密过程吗？ (y/N): ")
            if confirm.lower() == 'y':
            ...
            simulator.list_encrypted_files()
...
if __name__ == "__main__":
    # 使用说明
    print("使用方法:")
    print("  python3 script.py                    # 使用默认图标")
    print("  python3 script.py --icon /path/to/icon.jpg    # 指定锁图标")
    print()
